...
bkcore.hexgl.ShipControls.prototype.update = function(dt)
{
	...
	var dir;
	if (undefined != hand){
	//	console.log("x:" + handx + "y:" + handy + "z:" + handz);
        ...
        var left = 0; 
        var right = 0; 
        for (var i = 0; i < thisframe.pointables.length; i++) {
        	if (thisframe.pointables[i].direction[0]>0.05){left = left+1;}
        	if (thisframe.pointables[i].direction[0]<.05){right = right+1;}
        }
	 	...
	 	if ((thisframe.pointables.length > 2) && Math.abs(right - left) > 1){
	 		if (left >= thisframe.pointables.length-2){
	 			this.key.right = true;
	 			this.key.left = false;
	 		}
	 		else if (right >= thisframe.pointables.length-2){
