...
{  
	...
	if (undefined != hand){
        ...
        var right = 0; 
	 	...
	 	if (hand.palmNormal[0] < -.35 ){
	 		this.key.right = true;
	 		queue.push("right");  
	 		leftMovementMode = false;  
	 		this.key.left = false;
	 	}
	 	else if (hand.palmNormal[0] > .35 ){
	...
	if(pitchAng>0)
		this.speed += this.thrust * dt * pitchAng;   //accelerate more based on pitch ang
	else
		this.speed += this.airResist * dt * pitchAng; //the hand will break
	if(this.key.left)
	{
