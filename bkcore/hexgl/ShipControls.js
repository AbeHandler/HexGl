...
bkcore.hexgl.ShipControls = function(domElement)
{
	...
	this.airAngularSpeed = 0.0005;
	this.repulsionRatio = 0.5;
	this.repulsionCap = 2.5;
	this.repulsionLerp = 0.1;
	this.collisionSpeedDecrease = 0.9;
	this.collisionSpeedDecreaseCoef = 0.9;
	this.maxShield = 1.0;
	this.shieldDelay = 60;
	this.shieldTiming = 0;
	this.shieldDamage = 0.25;
	this.driftLerp = 0.35;
	this.angularLerp = 0.15;
...
{  
	...
	if (undefined != hand){
        ...
        var right = 0; 
	 	...
	 	if (hand.palmNormal[0] < -.05 ){
	 		this.key.right = true;  
	 		this.key.left = false;
	 	}
	 	else if (hand.palmNormal[0] > .05 ){
	...
	if(pitchAng>0){
		this.speed += this.thrust * Math.abs(pitchAng) * dt;   //accelerate more based on pitch ang
	}
	else{
		this.speed -= this.airResist * dt; //the hand will break
	}
	if(this.key.left)
	{
		angularAmount += this.angularSpeed * dt * (Math.abs(hand.palmNormal[0]));
