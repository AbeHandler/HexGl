...
var queue = [];  
var lastRoll = 0;
var rightMovementMode = false;
var leftMovementMode = false;
var counter = 0;
var handx;
var handy;
var handz;
...
{  
	...
	//if the hand velocity does not change at all then
	//the hand is "stuck" on the last frame. Reset loop
	if (undefined != hand){
		if (this.lastFrameXVelocity == hand.palmVelocity[0]){
			hand = undefined;
		}//
	}
	...
	if (undefined != hand){
		...
		pitchAng = Math.atan2( hand.direction[1] , -hand.direction[2] );
		...
		yaw = Math.atan2(hand.direction[0] , -hand.direction[1]);
        ...
        this.lastFrameXVelocity = hand.palmVelocity[0]; 

	 	if (hand.palmVelocity[0] > 100 ){
	 		this.key.right = true;
	 		queue.push("right");  
	 		leftMovementMode = false;  
	 		this.key.left = false;
	 	}
	 	else if (hand.palmVelocity[0] < -100 ){
	 		this.key.right = false;
	 		this.key.left = true;
	 		queue.push("left");  
	 		rightMovementMode = false;   
	 	}  
	 	else if (rightMovementMode){   //if you are in rightMovementmode, then pitch keeps turning it
	 		if (roll < -.45){
	 			this.key.right = true;
	 			this.key.left = false;
	 		}
	 		if (Math.abs(roll) < .1 && Math.abs(lastRoll) < .1){
	 			rightMovementMode = false; //flatten palm turns off mode
	 		}
	 	}
	 	else if (leftMovementMode){
	 		if (roll > .45){
	 			this.key.right = false;
	 			this.key.left = true;
	 		}
	 		if (Math.abs(roll) < .1 && Math.abs(lastRoll) < .1 ) {
	 			leftMovementMode = false; //flatten palm turns off mode
	 		}
	 	}
	 	else{
	 		this.key.right = false;
	...
	} else{   //if the hand is undefined the controls should not be L or R
		this.key.right = false;
	 	this.key.left = false;
	 	queue = []; 
	}
	...
	if (queue.length > 4){
		...
		var item3 = queue[3];
		var item4 = queue[4];
		if (item=="right" && item1=="right" && item2=="right" && item3 =="right" && item4 =="right"){
			rightMovementMode = true;
			leftMovementMode = false;
		}
		if (item=="left" && item1=="left" && item2=="left" && item3 =="left" && item4 == "left"){
			leftMovementMode = true;
			rightMovementMode = false;
		}
		queue = [ queue[1], queue[2], queue[3], queue[4]]; //reset the queue
	}
